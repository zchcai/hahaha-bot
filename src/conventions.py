...
def evaluate(snapshot, actions):
    sorted_actions = sorted(actions, key=lambda action: _evaluate_action(snapshot, action), reverse=True)
    return sorted_actions

def _evaluate_action(snapshot, action):
...
PROMPT="""Now, to play this game in a professional way, please follow the conventions below:
...
"""
    prompt = HANABI_RULES_PROMPT + PROMPT + text + POST_PROMPT
    try:

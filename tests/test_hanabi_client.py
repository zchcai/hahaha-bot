...
def get_default_game_state():
    ...
    actions = [
        # Player 0 (self)
        Action(
            action_type=ACTION.DRAW.value, player_index=0, card=Card(order=0)
        ),  # discard slot
...
class TestHandleAction(unittest.TestCase):
    ...
    def test_record_play_clue(self, mock_websocketapp):
        ...
        data = {
            "type": "clue",
            "clue": {"type": ACTION.COLOR_CLUE.value, "value": 1},
            "giver": 1,
            "list": [0, 3],
            "target": 0,
            "turn": 0,
...
class TestDecideAction(unittest.TestCase):
    ...
    @patch("websocket.WebSocketApp")
    def test_play_card_when_no_emergency_but_with_playable_card_to_clue(
        self, mock_websocketapp
    ):
        """When play clue is given."""
    ...
    def test_do_not_give_save_clue_when_player_is_busy(self, mock_websocketapp):
        ...
        # They have a play.
        state.player_hands[1][1].clues = [
            Clue(hint_type=2, hint_value=3, classification=1)
        ]
        # We also have a play.
        state.player_hands[0][1].clues = [
            Clue(hint_type=2, hint_value=4, classification=1)
        ]
        client = get_default_client(state)
    ...
    def test_play_deduced_playable_card(self, mock_websocketapp):
        ...
        for i in range(2, 6):
            state.player_hands[0][0].add_negative_info(
                Clue(hint_type=ACTION.RANK_CLUE.value, hint_value=i)
            )
        for i in range(1, 5):
            state.player_hands[0][0].add_negative_info(
                Clue(hint_type=ACTION.COLOR_CLUE.value, hint_value=i)
            )
        client = get_default_client(state)
